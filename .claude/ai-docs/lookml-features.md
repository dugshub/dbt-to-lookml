...
Groups multiple timezone representations (UTC, local) into a single toggleable dimension_group.
...
dimensions:
  - name: starts_at
    ...
    config:
      meta:
        timezone_variant:
          canonical_name: "starts_at"  # Shared identifier
          variant: "utc"               # Matches column suffix
          is_primary: true             # This becomes the toggleable dimension
    ...
    config:
      meta:
        timezone_variant:
          ...
          variant: "local"
          is_primary: false            # Excluded from output
```
...
- `_generate_toggleable_dimension_group()`: Generates dimension with Liquid injection
- `_has_timezone_variants()`: Quick check for toggleable variants

### Explore always_filter

When timezone variants exist, explores include `always_filter` to show the timezone selector:
