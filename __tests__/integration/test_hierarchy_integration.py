...
class TestHierarchyIntegration:
    ...
    def test_end_to_end_hierarchy_generation(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            booking_status_dim = next(
                (d for d in dimensions if d['name'] == 'booking_status'), 
                None
            )
            ...
            facility_type_dim = next(
                (d for d in dimensions if d['name'] == 'facility_type'), 
                None
            )
            ...
            created_date_group = next(
                (d for d in dimension_groups if d['name'] == 'created_date'), 
                None
            )
            ...
            revenue_measure = next(
                (m for m in measures if m['name'] == 'total_revenue'), 
                None
            )
            ...
            count_measure = next(
                (m for m in measures if m['name'] == 'booking_count'), 
                None
            )
            ...
            avg_measure = next(
                (m for m in measures if m['name'] == 'avg_lead_time'), 
                None
            )
    ...
    def test_mixed_hierarchy_and_no_hierarchy(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            dimensions = view.get('dimensions', [])
            with_hierarchy = next(
                (d for d in dimensions if d['name'] == 'with_hierarchy'), 
                None
            )
            ...
            without_hierarchy = next(
                (d for d in dimensions if d['name'] == 'without_hierarchy'), 
                None
            )
            ...
            measures = view.get('measures', [])
            measure_with = next(
                (m for m in measures if m['name'] == 'measure_with_hierarchy'), 
                None
            )
            ...
            measure_without = next(
                (m for m in measures if m['name'] == 'measure_without_hierarchy'), 
                None
            )
