...
class TestErrorHandling:
    ...
    def test_parser_invalid_field_types(self) -> None:
        ...
        invalid_models = [
            ...
            {"name": "test", "model": "table", "dimensions": [["not", "a", "dict"]]},
        ]
    ...
    @pytest.mark.skip(
        reason="SemanticModelMapper no longer exists - functionality moved to schemas"
    )
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                if validation_errors:
                    ...
                    assert any(
                        "permission" in error.lower() or "error" in error.lower()
    ...
    def test_generator_output_cleanup_on_error(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            with patch.object(
                generator, "_generate_view_lookml", side_effect=failing_generate
            ):
                generated_files, validation_errors = generator.generate_lookml_files(
                    models, output_dir
                )
    ...
    def test_format_with_malformed_lookml(self) -> None:
        ...
        for content in malformed_contents:
            try:
                ...
                # Should not raise unexpected exceptions
                assert "format" in str(e).lower() or isinstance(
                    e, (IndexError, AttributeError)
                )
