        ...
        try:
            ...
            for entity_data in model_data.get("entities", []):
                try:
                    ...
                    raise ValueError(
                        f"Error parsing entity '{entity_data.get('name', 'unknown')}': {e}"
                    ) from e
            ...
            for dim_data in model_data.get("dimensions", []):
                try:
                    ...
                    raise ValueError(
                        f"Error parsing dimension '{dim_data.get('name', 'unknown')}': {e}"
                    ) from e
            ...
            for measure_data in model_data.get("measures", []):
                try:
                    ...
                    raise ValueError(
                        f"Error parsing measure '{measure_data.get('name', 'unknown')}': {e}"
                    ) from e
